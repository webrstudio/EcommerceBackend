...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!isset($data['usuario_nombre'], $data['usuario_email'], $data['usuario_telefono'], $data['usuario_direccion'], $data['usuario_carrito']) || !is_array($data['usuario_carrito'])) {
        ...
        $stmtUser = $pdo->prepare("INSERT INTO usuarios (usuario_nombre, usuario_email, usuario_telefono, usuario_direccion) VALUES (:usuario_nombre, :usuario_email, :usuario_telefono, :usuario_direccion)");
        ...
        $stmtPedido = $pdo->prepare("INSERT INTO pedidos (usuario_id, producto_id, producto_fecha) VALUES (:usuario_id, :producto_id, :producto_fecha)");

        foreach ($data['usuario_carrito'] as $item) {
            if (!isset($item['producto_id'], $item['producto_fecha'])) {
            ...
            $stmtPedido->bindParam(":producto_fecha", $item['producto_fecha']);
            $stmtPedido->execute();
        }
